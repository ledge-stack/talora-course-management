...
export default function SidebarNav({ userRole = 'Student', unreadCount = 0 }: { userRole?: string, unreadCount?: number }) {
  ...
  const navLinkClass = (href: string) =>
    `flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
      isActive(href)
        ? 'bg-primary-transparent text-primary border-l-4 border-primary pl-[calc(1rem-4px)]'
  ...
  return (
    <>
      <nav className="flex-1 px-4 py-6 flex flex-col gap-1 overflow-y-auto">
        ...
        {isAdmin && (
          <>
            <div className="mt-4 mb-2 px-4 text-xs font-semibold text-text-muted uppercase tracking-wider">Platform Admin</div>
      ...
      <div className="px-4 py-6 border-t border-border-subtle flex flex-col gap-2">
        <ThemeToggle />
        ...
        <Link href="/notifications" className={navLinkClass('/notifications')}>
          <span className="w-full flex items-center justify-between">
            ...
            {unreadCount > 0 && (
              <span className="bg-primary text-white rounded-full px-2 py-0.5 text-[0.65rem] font-bold">{unreadCount}</span>
            )}
          </span>
        </Link>
